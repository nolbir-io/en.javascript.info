...
Tasavvur qiling, siz eng yaxshi qo'shiqchisiz va muxlislar kecha-yu kunduz bo'lajak qo'shig'ingizni so'rashadi.

Bir oz yengillik olish uchun siz uni chop etilganda ularga yuborishga va'da berasiz. Muxlislaringizga ro'yxat berasiz. Ular o'zlarining elektron pochta manzillarini to'ldirishlari mumkin, shunda qo'shiq mavjud bo'lganda, barcha obuna bo'lganlar uni bir zumda olishadi. Agar biror narsa noto'g'ri bo'lsa ham, aytaylik, studiyada yong'in sodir bo'lsa, qo'shiqni nashr eta olmasligingizdan ular baribir xabardor qilinadi.

Hamma xursand, chunki xalq sizni boshqa olomonga to'plamaydi, muxlislar esa qo'shiqni sog'inmaydilar.
...
`New promise` ga o'tkazilgan funksiya *executor* (ijrochi) deb ataladi. `New promise` yaratilganda, ijrochi avtomatik ravishda ishlaydi. U oxir-oqibat natija berishi kerak bo'lgan ishlab chiqarish kodini o'z ichiga oladi. Yuqoridagi oʻxshatish nuqtai nazaridan: ijrochi “qo'shiqchi”. 

Uning `resolve` va `reject` argumentlari JavaScriptning o'zi tomonidan taqdim etilgan qayta qo'ng'iroqlardir. Bizning kodimiz faqat ijrochining ichida joylashgan.
...
"Muxlislar" bu o'zgarishlarga qanday obuna bo'lishi mumkinligini keyinroq bilib olamiz.
...
2. Ijrochi ikkita argumentni oladi: `resolve` va `reject`. Bu funksiyalar JavaScript dvigateli tomonidan oldindan belgilangan, shuning uchun ularni yaratishimiz shart emas. Biz tayyor bo'lganda ulardan birini chaqirishimiz kerak.
   ...
   Bir soniya "qayta ishlash"dan so'ng, ijrochi natijani chiqarish uchun `resolve("done")` ni chaqiradi. Bu `promise` obyektining holatini o'zgartiradi:

    ![](promise-resolve-1.svg)

Bu ishni muvaffaqiyatli yakunlash,"fulfilled promise" (bajarilgan va'da) misoli edi.
...
`"Reject(...)` qo'ng'irog'i va'da obyektini `rejected` holatiga o'tkazadi:
...
````smart header="Darhol `resolve/reject` ni chaqirish`"
...
Misol uchun, bu biz ishni bajarishni boshlaganimizda sodir bo'ladi, lekin keyin hamma narsa allaqachon tugallangan va keshlanganligini ko'rishimiz mumkin.

Juda soz. Bizda darhol hal qilingan va'damiz bor.
````
...
Agar bizni faqat muvaffaqiyatli yakunlash qiziqtirsa, `.then` ga faqat bitta funksiya argumentini taqdim etishimiz mumkin:
...
Agar bizni faqat xatolar qiziqtirsa, birinchi argument sifatida `null` dan foydalanishimiz mumkin: `.then(null, errorHandlingFunction)`. Yoki biz `.catch(errorHandlingFunction)` dan foydalanishimiz ham mumkin, bu aynan bir xil hisoblanadi:
...
`.catch(f)` chaqiruvi `.then(null, f)` ning to'liq analogidir, bu shunchaki qisqartma.
...
Buni partiyani tugatuvchi sifatida tasavvur qiling. Bayram yaxshi yoki yomon bo'lishi va unda qancha do'stlar bo'lishidan qat'iy nazar, biz undan keyin tozalashga muhtojmiz (yoki hech bo'lmaganda). 
...
2. `Finally` ishlov beruvchisi natija yoki xatoni keyingi mos ishlov beruvchiga "o'tadi".
    ...
    new Promise((resolve, reject) => {
      ...
      .finally(() => alert("Promise ready")) // birinchi bo'lib harakatga keladi
    ...
    Ko'rib turganingizdek, birinchi promise bilan qaytarilgan `value` `finally` orqali keyingi `then` ga o'tadi.
     ...
     Bu juda qulay, chunki `finally` va'da qilingan natijani qayta ishlash uchun mo'ljallanmagan. Aytganimizdek, natija qanday bo'lishidan qat'iy nazar, bu umumiy tozalash uchun joy. 
...
Bu funksiyalar foydali. Agar biz `finally` dan to'g'ri foydalansak, ishlarning to'g'ri ishlashiga umumiy tozalash protseduralari uchun yordam beradi.
...
Promise lar yanada moslashuvchan. Biz istalgan vaqtda ishlov beruvchilarni qo'shishimiz mumkin: agar natija allaqachon mavjud bo'lsa, ular shunchaki ishlaydi.
````

## Masalan: loadScript [#loadscript]

Keyinchalik, keling, promise'lar asinxron kod yozishga qanday yordam berishi haqida ko'proq amaliy misollarni ko'rib chiqaylik.
...
Quyida undan foydalanish berilgan:
...
Callback ka asoslangan patern ning bir nechta afzalliklarini darhol ko'rishimiz mumkin:
...
| Biz `.then` ga va'da bo'yicha xohlagancha qo'ng'iroq qilishimiz mumkin. Har safar biz “obuna roʻyxati”ga yangi “fanat”, yangi obuna funksiyasini qo'shamiz. Bu haqda keyingi bobda bilib olamiz: [](ma'lumot: promise zanjiri). | Faqat bitta callback mavjud bo'lishi mumkin. |

Shunday qilib, va'dalar bizga yaxshiroq kod oqimi va moslashuvchanlikni beradi. Ammo hali bundan ham ko'proq foydali narsa mavjud. Buni keyingi boblarda ko'ramiz.
